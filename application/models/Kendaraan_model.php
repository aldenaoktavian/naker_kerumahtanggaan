...
class Kendaraan_model extends CI_Model {
	...
	{
		if($search != ''){
			$query = $this->db->query("SELECT count(*) FROM kendaraans a INNER JOIN jenis_kendaraans b ON a.jns_kendaraan_id = b.id WHERE DATE_FORMAT(NOW(),'%Y-%m-%d')= DATE_SUB(DATE_FORMAT(masa_stnk,'%Y-%m-%d'), INTERVAL 5 MONTH) AND (
					b.nama_jenis LIKE '%".$search."%' OR
					a.kode_kendaraan LIKE '%".$search."%' OR 
					a.merk LIKE '%".$search."%' OR 
					a.tahun LIKE '%".$search."%' OR 
			...
			$query = $this->db->query("SELECT a.*, b.kode_jenis, b.nama_jenis FROM kendaraans a INNER JOIN jenis_kendaraans b ON a.jns_kendaraan_id = b.id WHERE DATE_FORMAT(NOW(),'%Y-%m-%d')= DATE_SUB(DATE_FORMAT(masa_stnk,'%Y-%m-%d'), INTERVAL 5 MONTH)")->row_array();
		}
		return $query['jml'];
	}
}
